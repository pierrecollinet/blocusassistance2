    ...
    #    guest_email_id = request.session.get('guest_email_id')
        ...
        if user.is_authenticated():
            ...
            obj, created = self.model.objects.get_or_create(
                            user=user, email=user.email) # ATTENTION USER HAVE TO HAVE AN EMAIL
...
class BillingProfile(models.Model):
    ...
    def charge(self, inscription_obj, card=None):
        return Charge.objects.do(self, inscription_obj, card)
...
class ChargeManager(models.Manager):
    def do(self, billing_profile, inscription_obj, card=None): # Charge.objects.do()
        card_obj = card
        if card_obj is None:
        ...
        c = stripe.Charge.create(
              ...
              metadata={"inscription_id":inscription_obj.id},
            )
